...
class _MoodQuestionsScreenState extends State<MoodQuestionsScreen>
  ...
  // === NEW: Track current questions per index ===
  late List<Question> _currentQuestions;
  late List<Set<int>> _usedIndices; // to avoid repeating questions
  ...
  void _initializeQuestions() {
    ...
    for (int i = 0; i < allQuestions.length; i++) {
      ...
      _usedIndices.add({i}); // mark original as used
    }
  }
  ...
  void _initAnimations() {
    ...
    final double staggerAmount =
        _currentQuestions.length > 1 ? 0.8 / (_currentQuestions.length - 1) : 0.0;
    ...
    for (int i = 0; i < _currentQuestions.length; i++) {
      ...
      final slide = Tween<Offset>(begin: const Offset(0, 0.6), end: Offset.zero).animate(
        CurvedAnimation(
          parent: _staggerController,
          curve: Interval(start, 1.0, curve: Curves.easeOutCubic),
        ),
  ...
  void _replaceQuestion(int index) {
    ...
    if (available.isEmpty) {
      // All questions used → allow repeat (rare case)
    ...
    setState(() {
      ...
      // Rebuild animation for this card only
      _rebuildAnimationForIndex(index);
    });
  }

  void _rebuildAnimationForIndex(int index) {
    final fade = Tween<double>(begin: 0.0, end: 1.0).animate(
      CurvedAnimation(
        parent: _staggerController,
    ...
    _fadeAnimations[index] = fade;
    _slideAnimations[index] = slide;

    // Restart animation for this card
        ...
        return SlideTransition(
          ...
          child: FadeTransition(
            ...
            child: Transform.scale(
              scale: 0.96 + (0.04 * _fadeAnimations[index].value),
              child: Opacity(
                ...
                child: Container(
                  ...
                  decoration: BoxDecoration(
                    ...
                    border: Border.all(
                      color: widget.moodColor.withOpacity(0.3),
                      width: 1.2,
                    ),
                    boxShadow: [
                      BoxShadow(
                    ...
                    children: [
                      ...
                      if (question.type == QuestionType.text || question.type == QuestionType.number)
                        TextField(
                          ...
                          style: _poppins.copyWith(fontSize: 14, color: const Color(0xFF475569)),
                          decoration: InputDecoration(
                            hintText: question.hint.isNotEmpty ? null : 'Type your answer...',
                            hintStyle: _poppins.copyWith(fontSize: 14, color: const Color(0xFF94A3B8)),
                            filled: true,
                        ...
                        Row(
                          children: [
                            _buildToggleButton('Yes', true, index),
                            const SizedBox(width: 12),
                            _buildToggleButton('No', false, index),
                          ],
                        )
                          ...
                          children: question.options.asMap().entries.map((e) {
                            ...
                            return GestureDetector(
                              ...
                              child: Container(
                                ...
                                decoration: BoxDecoration(
                                  color: isSelected ? widget.moodColor : const Color(0xFFF1F5F9),
                                  borderRadius: BorderRadius.circular(12),
                                  border: Border.all(
                                    color: isSelected ? widget.moodColor : const Color(0xFFE2E8F0),
                                  ),
                                ),
                                child: Text(
                                  option,
  ...
  Widget _buildToggleButton(String label, bool value, int index) {
 ...
 Future<void> _submitAnswers() async {
  ...
  try {
    ...
    if (response.statusCode == 201) {
      ...
      // الرجوع للرئيسية بدون فتح أي شاشة
      ...
      child: InkWell(
        ...
        child: Container(
          ...
          decoration: BoxDecoration(
            ...
            boxShadow: [
              BoxShadow(color: Colors.black.withOpacity(0.1), blurRadius: 12, offset: const Offset(1, 3)),
            ],
          ),
